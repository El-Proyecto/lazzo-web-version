  ...
  static Future<Uint8List> compressToWebP(XFile input) async {
    try {
      ...
      while (compressedBytes.length > maxBytes && currentQuality > minQuality) {
        currentQuality -= 5;
        print('   🔄 Reducing quality to $currentQuality% (current: ${_formatBytes(compressedBytes.length)})');
        compressedBytes = _encodeOptimized(resizedImage, currentQuality);
      }
      
      final finalSize = compressedBytes.length;
      final compressionRatio = ((originalSize - finalSize) / originalSize * 100).toStringAsFixed(1);
      ...
      if (finalSize > maxBytes) {
        print('   ⚠️ Warning: Unable to compress below ${_formatBytes(maxBytes)}');
      }
